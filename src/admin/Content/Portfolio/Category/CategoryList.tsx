...
const CategoryList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCategories = async (page = 1, search = '') => {
      ...
      try {
        const response = await fetch(`http://localhost:5002/api/portfolio/category/all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`, {
          headers: {
  ...
  const handleDelete = async (serviceId: number) => {
    if (window.confirm('Ești sigur că vrei să ștergi acest serviciu?')) {
      try {
        ...
        setCategories((prev) => prev.filter((category) => category.id !== serviceId));
      } catch (err) {
          ...
          <div className="overflow-x-auto">
              ...
              {categories.map((category) => {
                ...
                return (
                    <tr key={category.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-500" dangerouslySetInnerHTML={{ __html: category.description }} />
                      </td>
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-500 ms-4">
                Total servicii găsite: {totalCategories}
              </div>
